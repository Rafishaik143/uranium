...
const bookList = async function (req, res) {
    let bookName = await BookModel.find().select({ bookName: 1, authorName: 1, _id: 0 })
    res.send({ msg: bookName })
}

const booksinYear = async function (req, res) {
    let booksinYear = await BookModel.find({year:2020})
...
const inrBooks = async function (req, res) {
    ...
    let inrBooks = await BookModel.find({ 'prices.indianPrice': { $in: ["1000INR", "500INR", "200INR"] } })
    res.send({ msg: inrBooks })
}

const randomBooks = async function (req, res) {

    let randomBooks = await BookModel.find({$or:[{stockAvailable:false},{totalPages:{$gt:200}}]})
